...
class GraphicDisplay(tk.Tk):
    ...
    #     # 그리드 생성
    #     for col in range(0, WIDTH * UNIT, UNIT):  # 0~400 by 100
    #         x0, y0, x1, y1 = col, 0, col, HEIGHT * UNIT
    #         canvas.create_line(x0, y0, x1, y1)
    #     for row in range(0, HEIGHT * UNIT, UNIT):  # 0~400 by 100
    ...
    # 캔버스에 이미지 추가 (사각형, 원, 삼각형들)
        self.rectangle = canvas.create_image(50, 50, image=self.shapes[0])  # 사각형 (에이전트)
        canvas.create_image(250, 150, image=self.shapes[1])  # 기존 삼각형 1
        canvas.create_image(150, 250, image=self.shapes[1])  # 기존 삼각형 2
        canvas.create_image(250, 250, image=self.shapes[2])  # 원

    # 추가할 삼각형 이미지 (원하는 좌표에 배치)
        canvas.create_image(350, 150, image=self.shapes[1])  # 새로운 삼각형 1
        canvas.create_image(150, 350, image=self.shapes[1])  # 새로운 삼각형 2
    ...
    def load_images(self):
        # 업로드한 이미지 경로로 수정
        up = PhotoImage(Image.open('C:/Users/dusil/OneDrive/바탕 화면/3-2 강의자료/딥러닝과 강화학습/up.png').resize((13, 13)))
        right = PhotoImage(Image.open('C:/Users/dusil/OneDrive/바탕 화면/3-2 강의자료/딥러닝과 강화학습/right.png').resize((13, 13)))
        left = PhotoImage(Image.open('C:/Users/dusil/OneDrive/바탕 화면/3-2 강의자료/딥러닝과 강화학습/left.png').resize((13, 13)))
        down = PhotoImage(Image.open('C:/Users/dusil/OneDrive/바탕 화면/3-2 강의자료/딥러닝과 강화학습/down.png').resize((13, 13)))
        rectangle = PhotoImage(Image.open('C:/Users/dusil/OneDrive/바탕 화면/3-2 강의자료/딥러닝과 강화학습/rectangle.png').resize((65, 65)))
        triangle = PhotoImage(Image.open('C:/Users/dusil/OneDrive/바탕 화면/3-2 강의자료/딥러닝과 강화학습/triangle.png').resize((65, 65)))
        circle = PhotoImage(Image.open('C:/Users/dusil/OneDrive/바탕 화면/3-2 강의자료/딥러닝과 강화학습/circle.png').resize((65, 65)))
    ...
    def reset(self):
        if self.is_moving == 0:
            ...
            self.agent.policy_table[2][2] = []  # 목표 상태는 정책 없음
    ...
    def move_by_policy(self):
        if self.improvement_count != 0 and self.is_moving != 1:
            ...
            while len(self.agent.policy_table[x][y]) != 0:
                best_action = self.agent.get_action([x, y])
                self.after(100, self.rectangle_move(best_action[0]))  # 첫 번째 행동 수행
                x, y = self.find_rectangle()
            self.is_moving = 0
    ...
    def improve_policy(self):
        ...
        for i in range(HEIGHT):
            for j in range(WIDTH):
                state = [i, j]
                best_action = self.agent.get_action(state)  # 각 상태에 대해 행동을 선택
                policy = [0, 0, 0, 0]  # Up, Down, Left, Right 초기화
                for action in best_action:
                    policy[action] = 1
                self.draw_one_arrow(i, j, policy)  # 행동에 대한 화살표 그리기


# 환경 클래스 추가
class Env:
    def __init__(self):
        self.transition_probability = 1.0  # 상태 전이 확률
        self.width = 5  # 그리드월드의 너비
        self.height = 5  # 그리드월드의 높이
        self.reward = [[0] * self.width for _ in range(self.height)]  # 보상 테이블
        self.possible_actions = [0, 1, 2, 3]  # 가능한 행동 (좌, 우, 상, 하)
        
        # 특정 좌표에 보상 설정
        self.reward[2][2] = 1  # (2,2) 좌표에 보상 1 (목표 지점)
        self.reward[1][2] = -1  # (1,2) 좌표에 보상 -1 (장애물)
        self.reward[2][1] = -1  # (2,1) 좌표에 보상 -1 (장애물)
        self.reward[3][1] = -1
        self.reward[1][3] = -1
    ...
    # 현재 상태에서 행동을 취한 후 다음 상태와 보상을 반환
    def get_reward(self, state, action):
        next_state = self.state_after_action(state, action)
        return self.reward[next_state[0]][next_state[1]]  # 다음 상태에서의 보상 반환

    # 현재 상태에서 행동을 한 후의 다음 상태를 반환
    def state_after_action(self, state, action_index):
        action = self.action_to_delta(action_index)  # 행동을 좌표 변화로 변환
        next_state = [state[0] + action[0], state[1] + action[1]]
        return self.check_boundary(next_state)  # 경계를 넘지 않도록 조정된 상태 반환

    # 행동을 좌표 변화로 변환 (상, 하, 좌, 우)
    def action_to_delta(self, action_index):
        if action_index == 0:  # 상
            return [-1, 0]
        elif action_index == 1:  # 하
            return [1, 0]
        elif action_index == 2:  # 좌
            return [0, -1]
        elif action_index == 3:  # 우
            return [0, 1]

    # 경계 검사 및 상태 조정 (경계를 넘어가면 경계에 머무름)
    def check_boundary(self, state):
        state[0] = max(0, min(state[0], self.height - 1))  # 세로 경계 검사
        state[1] = max(0, min(state[1], self.width - 1))   # 가로 경계 검사
        return state

    # 상태 전이 확률 반환 (이 예제에서는 항상 1로 고정)
